class MeshRenderer {
    constructor(gl /*= new WebGLRenderingContext*/, data = {}, vS = "", fS = "") {

        if (vS instanceof WebGLProgram)
            this.program = vS
        else if (!hasAllStrs( vS, ['uProj', 'uView', 'uWorld', 'aPos']))
            return new Error("One of obrigatory shader uniforms or attributes are not present in the shader")
        else
        ...
        this.normalData = new Float32Array(data.normals)
        ...
        if (this.useNormals = !!this.normalData.length) {
    ...
    render(vm, pm) {
        //Here i check if is equal to true coz it will generate more efficient bytecode in v8 and SpiderMonkey engine.
        //BROWSER MASTERRACE INSENSIFIES
